//HELLO IF YOU'RE VIEWING THIS FROM THE MASTER BRANCH, PLEASE EXCLUDE SINCE WE ARE NOT USING MONGODB TO STORE OUR POSTS.
...
class Posts {
  constructor(title, image, overview, content, date, realDate) {
    //in this constructor, we want to store the post information:
    ((this.title = title),
      (this.image = image),
  ...
  save() {
    ...
    return db
      .collection("posts")
      .insertOne(this) //we are returning the collection becacuse it will allow us to treat this whole chain as a promise.
      .then((result) => {
        // console.log(result);
      })
      .catch((err) => {
